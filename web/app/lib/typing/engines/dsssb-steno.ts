/**
 * DSSSB Stenographer Typing
 * --------------------------------------------------------------------------
 * 10 minutes · English 40 WPM / Hindi 30 WPM.
 * Backspace: ENABLED.
 * Retype: Single Pass (Disabled).
 *
 * Matching TypingMitra DSSSB Stenographer rules:
 ...
 * - Qualification: Net WPM ≥ 40 WPM (English) or ≥ 30 WPM (Hindi).
 */
...
export const config: CategoryConfig = {
  ...
  retypeAllowed: false,
  language: 'both',
  notes: 'All errors count as full error (1 error). 2-word penalty per error. Minimum 40 WPM English / 30 WPM Hindi.',
};
...
export function evaluate(input: EngineInput): EngineResult {
  ...
  return finalize({
    ...
    input,
    analysis: a,
    badge: 'DSSSB Stenographer — All errors full, 2-word penalty per error',
    netWpm,
    netWords,
    errorPercentage,
    penaltyWords,
