...
async function bulkSeed() {
  ...
  try {
    console.log('1. Upserting all 39 Categories...');
    const allCategories = [];
    const allTests = [];
    ...
    // Insert categories in one bulk statement
    for (const cat of allCategories) {
      await client.query(`
        ...
        ON CONFLICT (id) DO UPDATE SET
          ...
          "updatedAt" = NOW();
      `, [cat.id, cat.name, cat.nameHi, cat.description, cat.icon, cat.logoUrl, cat.orderIndex, cat.isActive]);
    }
    console.log(`   ✅ Inserted/Updated ${allCategories.length} Categories!`);

    // Insert tests in chunks of 50
    console.log(`2. Inserting ${allTests.length} Typing Tests in chunks...`);
    const chunkSize = 50;
    for (let i = 0; i < allTests.length; i += chunkSize) {
      ...
      process.stdout.write(`   [${Math.min(i + chunkSize, allTests.length)}/${allTests.length}] tests saved...\r`);
    }

    console.log(`\n🎉 COMPLETED! All ${allCategories.length} categories and ${allTests.length} tests are active in database!`);
  } catch (err) {
