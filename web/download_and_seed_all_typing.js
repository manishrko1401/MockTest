...
function downloadImage(url, destPath) {
  return new Promise((resolve) => {
    ...
    if (fs.existsSync(destPath) && fs.statSync(destPath).size > 100) return resolve(destPath);

    const client = url.startsWith('https') ? https : http;
    const req = client.get(url, { headers: { 'User-Agent': 'Mozilla/5.0' }, timeout: 6000 }, (res) => {
...
// Ensure public/typing-logos directory exists
const logosDir = path.join(__dirname, 'public', 'typing-logos');
...
// Default passages library for generating comprehensive tests
...
async function seedAll() {
  console.log('==================================================');
  console.log('🚀 FAST SEEDING ALL 39 TYPING EXAM CATEGORIES');
  console.log('==================================================\n');

  // Step 1: Download all logos in parallel
  console.log('1. Downloading category logos in parallel...');
  const downloadPromises = scrapedData.map(async (item) => {
    let cleanSlug = item.slug.replace(/[^\w-]/g, '');
    let ext = path.extname(item.logoUrl.split('?')[0]) || '.webp';
    if (!ext.includes('.')) ext = '.webp';
    const safeFilename = `${cleanSlug}${ext}`;
    const localLogoPath = path.join(logosDir, safeFilename);
    const downloaded = await downloadImage(item.logoUrl, localLogoPath);
    item.resolvedLogoUrl = (downloaded || fs.existsSync(localLogoPath)) ? `/typing-logos/${safeFilename}` : item.logoUrl;
  });
  ...
  try {
    console.log('2. Ensuring Database schema and inserting categories...');
    ...
    for (let i = 0; i < scrapedData.length; i++) {
      ...
      // Encode extra metadata into clean description
      const metaBlock = {
      ...
      await client.query(`
        INSERT INTO typing_categories (id, name, "nameHi", description, icon, "logoUrl", "orderIndex", "isActive", "updatedAt")
      ...
      `, [
        ...
        item.name,
        item.name,
        JSON.stringify(metaBlock),
        'Keyboard',
        item.resolvedLogoUrl || item.logoUrl,
        i + 1,
        true
      ]);

      // Determine parameters based on exam type
      ...
      // Generate Passages for this Category across Easy, Medium, Hard (English + Hindi)
      const testsToInsert = [];

      // 1. English Easy Passages
      ...
      // Special Pangram
      if (categoryId.includes('quick-brown-fox')) {
        testsToInsert.unshift({
          id: `${categoryId}-pangram-drill`,
          title: `The Quick Brown Fox Pangram Drill`,
          titleHi: `द क्विक ब्राउन फॉक्स पैंग्राम अभ्यास`,
          passageText: `The quick brown fox jumps over the lazy dog. The quick brown fox jumps over the lazy dog. The quick brown fox jumps over the lazy dog. The quick brown fox jumps over the lazy dog. The quick brown fox jumps over the lazy dog.`,
          language: 'en',
          difficulty: 'Easy',
          mainDurationMinutes: 2,
          qualifyingWpm: 40,
      ...
      for (const t of testsToInsert) {
        await client.query(`
          INSERT INTO typing_tests (
            ...
            "highlightAllowed", language, difficulty, instructions, "orderIndex", "isActive", "updatedAt"
          ) VALUES (
        ...
        `, [
          ...
          1,
          1,
          t.mainDurationMinutes,
          t.qualifyingWpm,
          t.maxErrorPercentage,
          'ALLOWED',
          t.enableBackspace,
          t.allowRetype,
          false,
          t.language,
          t.difficulty,
          typingRules || `Official ${item.name} typing simulation. Required speed: ${t.qualifyingWpm} WPM.`,
          t.orderIndex,
          true
        ]);
      }
    ...
    console.log('   ✅ All 39 categories and tests inserted into Database successfully!');
  } catch (err) {
