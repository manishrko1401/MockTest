...
// Function to extract direct admit card download link from details page HTML
function extractDirectLink(pageHtml, defaultUrl) {
  ...
  // Search for admit card keywords in order of preference
...
async function main() {
  const htmlPath = 'C:\\Users\\painl\\.gemini\\antigravity-ide\\brain\\efd68c44-b16d-434b-9ee6-b9b7eda9d218\\scratch\\fetched_admit_cards.html';
  ...
  while ((match = loopItemRegex.exec(html)) !== null) {
    ...
    parsedCards.push({ title, url });
  }
  
  console.log(`Parsed ${parsedCards.length} admit cards from the main HTML.`);
  
  if (parsedCards.length === 0) {
    console.error("No admit cards could be parsed. Exiting.");
    return;
  }

  // Clear previous imported admit cards to ensure correct chronological sorting order
  console.log("Deleting old imported admit cards from Notice table...");
  const deleteResult = await prisma.notice.deleteMany({
    where: {
      category: 'admit_card'
    }
  });
  console.log(`Deleted ${deleteResult.count} previous admit card notices.`);
  ...
  for (let i = 0; i < parsedCards.length; i += BATCH_SIZE) {
    ...
    console.log(`Processing batch ${Math.floor(i / BATCH_SIZE) + 1}/${Math.ceil(parsedCards.length / BATCH_SIZE)} (items ${i + 1} to ${Math.min(i + BATCH_SIZE, parsedCards.length)})...`);
    
    const batchPromises = batch.map(async (card) => {
      try {
        const pageHtml = await fetchUrl(card.url);
        const schemaMatch = /"datePublished"\s*:\s*"([^"]*)"/i.exec(pageHtml);
        const directUrl = extractDirectLink(pageHtml, card.url);
    ...
    for (let j = 0; j < batch.length; j++) {
      ...
      if (res && res.publishDateObj) {
        ...
        cardsWithDates.push({
          ...
          directUrl: res ? res.directUrl : originalCard.url
        });
      }
    }
  ...
  let insertedCount = 0;
  for (let i = 0; i < cardsWithDates.length; i++) {
    const card = cardsWithDates[i];
    
    // Hash URL for unique ID
    const hash = crypto.createHash('md5').update(card.url).digest('hex').substring(0, 10);
    const id = `ac_${hash}`;
    
    // Assign createdAt timestamp with offset to guarantee sorting order
    const createdAtTimestamp = new Date(card.publishDateObj.getTime() - (i * 1000));
    ...
    await prisma.notice.create({
      data: {
        ...
        title: card.title,
        date: card.dateStr,
        publishDate: card.publishDateStr,
        type: 'ADMIT CARD',
        category: 'admit_card',
        url: card.directUrl || card.url, // Save direct admit card link!
  ...
  console.log(`Success! Imported ${insertedCount} admit card notices with accurate publication dates, direct download links, and correct sorting order.`);
}
