...
// Use the shorter/longer physical edge so the math holds regardless of orientation
// (the app is portrait-locked today, but this stays correct if that ever changes).
const SHORT_EDGE = Math.min(SCREEN_WIDTH, SCREEN_HEIGHT);
const LONG_EDGE = Math.max(SCREEN_WIDTH, SCREEN_HEIGHT);
const IS_TABLET = SHORT_EDGE >= 600;

// Responsive scale helpers.
// Previously these were a raw linear ratio off a 390×844 base with NO clamp, so a
// 768px tablet rendered every font/spacing value at ~2× (rs(15) → 30) and the whole
// terminal broke; a 320px phone got ~0.82× and text was cramped. Now the multiplier
// is clamped to a sane band — tablets get a small, deliberate bump instead of a
// runaway one, and tiny phones never drop below 0.92×.
const WIDTH_SCALE = Math.min(
  Math.max(SHORT_EDGE / 390, 0.92),
  IS_TABLET ? 1.18 : 1.12,
);
const HEIGHT_SCALE = Math.min(Math.max(LONG_EDGE / 844, 0.92), 1.15);

const rs = (size: number) => Math.round(size * WIDTH_SCALE);
const vs = (size: number) => Math.round(size * HEIGHT_SCALE);

// Max width for the question/option reading column. On phones this is the full
// width; on tablets it keeps line length readable instead of stretching edge-to-edge.
const CONTENT_MAX_WIDTH = IS_TABLET ? 760 : SCREEN_WIDTH;
...
export default function MobileTestScreen({
  ...
  // Active question details - fall back to Hindi if English content is empty (bilingual questions)
...
const styles = StyleSheet.create({
  ...
  questionContentContainer: {
    padding: rs(16),
    paddingBottom: vs(24),
    // Keep line length readable on tablets/large displays instead of stretching
    // question text and options edge-to-edge.
    width: '100%',
